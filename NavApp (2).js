...
function onLocationFound(e){
  ...
  if (destinationLatLng) {
    if (routeMode === "drive") {
      ...
      else requestGraphHopper(latlng, destinationLatLng, "foot");
    }
  }
  updateHUD();
}
...
async function ghRoute(from, to, profile = "foot", optionalVia = null){
  if (!GRAPHOPPER_KEY || GRAPHOPPER_KEY === "f4c5a097-3b80-4b50-88f7-8b6f8a115181") {
...
async function requestGraphHopper(from, to, profile = "foot", optionalVia = null){
  ...
  // Style: red dashed for walking; blue solid for driving
  const style = profile === "foot"
    ? { color:"#ef4444", weight:4, opacity:.95, dashArray:"8,8" }
    : { color:"#0b3a6e", weight:5, opacity:.95 };

  if (!routeLine) routeLine = L.polyline(r.coords, style).addTo(map);
  else { routeLine.setStyle(style); routeLine.setLatLngs(r.coords); }
...
function recomputeRoute(){
  if (!destinationLatLng || !userMarker){ updateHUD(); updateSteps(null); return; }
  const from = userMarker.getLatLng();
  const profile = (routeMode === "drive") ? "car" : "foot";
  requestGraphHopper(from, destinationLatLng, profile);}
